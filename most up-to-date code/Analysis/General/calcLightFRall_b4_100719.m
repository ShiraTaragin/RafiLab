function lp = calcLightFRall(lp, correctSniffModulation)
spikeTimes =[];
if nargin < 2
    correctSniffModulation = 0;
end
if correctSniffModulation
...
% calcLightFR
% output as a matrix that correspond to the 
% lightSpikes - FR during the light 
% nonLightSpikes -  FR during the non light  time
% allLigtSpikes structure with the actual spikes
...
%     lightOff = lightOn+lp.stimuliDuration; %TTLEvents(2,ii) + 0.02;
%     lightOff = lightOn + 0.12;
    lightOff = lightOn + max(lp.stimuliDuration, 0.05)+0.07;
    %lightOff = lightOn + 0.2;
    ...
    if correctSniffModulation & ~isempty(lightOnSniffIndx) & (lightOnSniffIndx)
    ...
    if correctSniffModulation & ~isempty(bglightOnSniffIndx) & (bglightOnSniffIndx)
