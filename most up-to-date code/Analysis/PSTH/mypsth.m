function [R,t,E] = mypsth( data,sig,plt,T,err,t)
limitPlotAxis=[0 0];  % set the left and right limits
noPathFlag = 0;
...
%if sz(1)>sz(2); 
    data=data';   % RAFI: in our data set the samples are always the columns
%end; 
...
% place all spikes time in one matrix D.     
m = 1;
D = zeros(size(data));
ND=zeros(1,NT);
for n=1:NT
...
% decide how many bins we use which is bin size = sig/5 
if nargin < 6
...
for n=1:NT
  for m=1:ND(n)
    RR(n,:) = RR(n,:) + exp(-f*(t-D(n,m)).^2);  % sum all spikes around the spike D(n,m). give normal weights to all 
  end
end
...
elseif err == 2
  ...
  E = sqrt((sE/Nboot - mE.^2/Nboot^2));  % variance is equal to E(x^2) - (E(x))^2
end
if err == 0
    hold on;
    noPathFlag=1;
...
if limitPlotAxis(1)
    x = find(t > limitPlotAxis(1)); 
    x = x(1);
    y = find(t < limitPlotAxis(2)); 
    y   = y(end);
...
if noPathFlag
    ...
    errorbar_patch(t,R,2*E,plt)
end
% Rafi:
% use this when the patch is not good
% plot(t,R,'color', plt)  
return
